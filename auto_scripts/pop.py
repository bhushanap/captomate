...
def romaji(line, l, io, effect_color, effect_outline_color,scale):
    for syl in Utils.all_non_empty(line.syls):
        ...
        l.text = (
            "{\\an5\\pos(%.3f,%.3f)"
            "\\t(0,%d,0.1,\\fscx%s\\fscy%s\\1c&H%s&\\3c&H%s&)"
            "\\t(%d,%d,0.3,\\fscx100\\fscy100)}%s"
            % (
                syl.center,
                syl.middle,
                l.dur / 3,
...
def overlay(user,effect_color, effect_outline_color, scale):
    ...
    for line in lines:
        # Generating lines
        romaji(line, line.copy(),io,effect_color,effect_outline_color, scale)
